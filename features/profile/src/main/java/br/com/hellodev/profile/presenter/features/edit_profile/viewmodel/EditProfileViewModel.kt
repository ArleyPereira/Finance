package br.com.hellodev.profile.presenter.features.edit_profile.viewmodel
...
class EditProfileViewModel(
    ...
    var state: StateFlow<EditProfileState> = _state

    init {
        getUser()
    }

    fun dispatchAction(action: EditProfileAction) {
        when (action) {
            is EditProfileAction.Update -> {
                update()
            }

            is EditProfileAction.OnTextFieldChanged -> {
                onTextFieldChanged(value = action.value, type = action.type)
            }

            is EditProfileAction.OnChangeImage -> {
                onChangeImage(uri = action.uri)
            }

            is EditProfileAction.DismissFeedback -> {
                dismissFeedback()
            }
        }
    }
